// Altura real del header (mantener padding del body)
...
function toggleMenu(force){
  ...
  document.body.classList.toggle('menu-open', willOpen); // lock scroll
}
...
// Año en footer (si existe el span#y)
const yEl=document.getElementById('y'); if(yEl) yEl.textContent=new Date().getFullYear();

// ===== Data de productos/ofertas (tu estructura original) =====
const productos=[
  ...
  <article class="card">
    ...
    <div class="body">
      <div class="tags">
        ...
        ${p.marca?`<span class="pill">${p.marca}</span>`:''}
      </div>
      <h3>${p.nombre}</h3>
      ${p.precio!=null?('<div class="price">$'+Number(p.precio).toFixed(2)+'</div>'):''}
...
function render(list){ if(grid) grid.innerHTML=list.map(cardHTML).join(''); }
function filtrar(){
