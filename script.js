// =======================================
// script.js — v21: Ofertas sí o sí + dots en móvil
// =======================================
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderOffers(){
    ...
    offersGrid.innerHTML=ofertas.map(cardHTML).join(''); // siempre pinta, aunque imagen falle
  ...
  // ===== Carrusel + dots =====
  const MAX_DOTS=5, state=new WeakMap();
  const isDesktop=()=>window.matchMedia('(min-width:900px)').matches;

  function ensureDotsEl(listEl){
    let dots=listEl.nextElementSibling && listEl.nextElementSibling.classList && listEl.nextElementSibling.classList.contains('dots')
      ? listEl.nextElementSibling : null;
    if(!dots){ dots=document.createElement('div'); dots.className='dots'; listEl.after(dots); }
    return dots;
  }
  function getSnaps(listEl){
    return Array.from(listEl.querySelectorAll('.card')).map(c=>Math.max(0,c.offsetLeft));
  }
  function nearestIndex(snaps,x){
    if(!snaps.length) return 0;
    let idx=0,best=Infinity;
    for(let i=0;i<snaps.length;i++){ const d=Math.abs(snaps[i]-x); if(d<best){best=d; idx=i;} }
    return idx;
  }
  ...
  function renderDots(listEl){
    ...
    const cardsCount=listEl.querySelectorAll('.card').length;

    // Desktop oculto por diseño, pero si no hay tarjetas, nada
    if(isDesktop() || cardsCount===0){
      dots.style.display='none';
      detach(listEl);
      return;
    }

    // Mostrar dots incluso con 1 tarjeta
    ...
    if(!s.onScroll){
      s.onScroll=()=>{ if(s.rafId) return; s.rafId=requestAnimationFrame(()=>{ s.rafId=null; renderDots(listEl); }); };
  ...
  function detach(listEl){
    const s=state.get(listEl); if(!s||!s.onScroll) return;
    ...
    s.onScroll=null; state.set(listEl,s);
  }
